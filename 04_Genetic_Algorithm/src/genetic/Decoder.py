...
def robot_decoder(genome: Genome, sensors: Sensors, prev_hidden: np.ndarray, prev_rotation: float) -> (float, float):
    ...
    input_nn = np.array([sensor.length for sensor in sensors.sensors] + [value[0] for value in prev_hidden] + [prev_rotation]).reshape(1, Const.INPUT_SIZE)
    ...
    hidden_nn = np.tanh(np.dot(np.append(input_nn, [1]), weights_input.T) ).reshape(1, Const.HIDDEN_SIZE)
    ...
    output_nn = np.tanh(np.dot(np.append(hidden_nn, [1]), weights_hidden.T)) * Const.MAX_SPEED
