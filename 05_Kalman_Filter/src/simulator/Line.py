...
class VisualLine:
    ...
    def __init__(self, origin: np.ndarray, dest: np.ndarray, dotted: bool, color: (int, int, int)):
        self.origin: np.ndarray = origin
        self.destination: np.ndarray = dest
        self.dotted: bool = dotted
        self.color: (int, int, int) = color

        if self.dotted:
            vec = np.array(self.destination - self.origin).reshape((2, 1))
            self.length = np.linalg.norm(vec)
            self.nvec = (vec / np.linalg.norm(vec)).reshape((2, 1))

        self.pyStart = pygame.Vector2(self.origin[0], self.origin[1])
        self.pyEnd = pygame.Vector2(self.destination[0], self.destination[1])

    def draw(self, screen):
        if not self.dotted:
            pygame.draw.line(screen, self.color, self.pyStart, self.pyEnd, 2)
        if self.dotted:
            # You know the drill https://codereview.stackexchange.com/questions/70143/drawing-a-dashed-line-with-pygame
            for index in range(0, int(np.round(self.length / Const.dotted_line)), 2):
                start = self.origin + (self.nvec * index * Const.dotted_line)
                end = self.origin + (self.nvec * (index + 1) * Const.dotted_line)

                pygame.draw.line(screen, self.color,  get_pygame_point(start), get_pygame_point(end), 2)
